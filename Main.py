...
from Features.freshMail import EmailReader
from Features.decrypt import SecretsDecryptor
    ...
    if prob.item() > 0.75:
        ...
        for intent in intents['intents']:
            ...
            if tag == intent["tag"]:
                ...
                return reply
# print(TasksExecutor("ping Tanmay"))
def MainTaskExecution (Query):
    ...
    try:
        if "open" in ReturnData:
            ...
            return True
        elif "mailCheck" in ReturnData:
            play_success_sound()
            email_reader = EmailReader(secrets["Email"],secrets['Password'])
            latest_emails = email_reader.get_latest_emails(2)

            if isinstance(latest_emails, str):
                Speak("No Unread E-Mails Today, Sir!")
            else:
                "unread mails"
                for email in latest_emails:
                    Speak('From: ' + email['From'])
                    Speak('Subject: '+ email['Subject'])
